...
```
'lsblk' 명령으로 모든 block device에 대한 정보들을 변수 'data_lsblk'에 저장했다. 이후 개별 데이터를 읽어와서, 앞서 말한 바와 같이 root directory 이하 경로를 Mount point로 가진 모든 block device에 대해 판별하였다. 판별한 이후에는 해당하는 정보에 대해 정규표현식을 사용해서 원하는 정보(disk usage)만을 추출했다. 이후 이 정보에 대해 다시 한번 90 이상인지 판별한 이후에, 해당 판별 결과에 따라 로그파일에 결과를 출력했다.
